...
export class TryOnService {
  ...
  constructor() {
    localQueue.register('tryon-render', (job) => this.onTryOnJob(job));
  }
  ...
  async executeTryOn(rawPayload: Record<string, unknown>) {
    ...
    const result = {
      ...
      sizeRec: {
        ...
        notes: [avatar ? 'Render tuned to avatar body metrics.' : 'Estimate based on provided height/weight.'],
    ...
    localQueue.enqueue('tryon-render', {
      tryOnId,
      avatarId: avatar?.id,
      productId: product.id,
      variantId: variant.id
  ...
  private onTryOnJob(job: QueueJob<{ tryOnId: string; avatarId?: string; productId: string; variantId: string }>) {
    const eventId = generateId();
    this.store.analyticsEvents.set(eventId, {
      id: eventId,
