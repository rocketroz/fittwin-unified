...
from app.core.validation import normalize_and_validate
from app.schemas.errors import ErrorResponse
from app.schemas.measure_schema import MeasurementInput, MeasurementNormalized
...
def verify_api_key(
    ...
    if x_api_key != VALID_API_KEY:
        legacy_path = request.url.path.startswith("/measurements")
        raise HTTPException(
            status_code=401 if legacy_path else 403,
...
async def validate_measurements(request: Request, input_data: MeasurementInput) -> dict:
    ...
    measurements_cm = {
        key: value for key, value in payload.items() if key.endswith("_cm")
    }
...
def recommend_sizes(measurements: MeasurementNormalized) -> dict:
    ...
    try:
        ...
        return {
            ...
            "recommended_size": primary.get("size") if primary else None,
