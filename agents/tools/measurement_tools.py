...
try:  # crewai is optional in some environments
    if os.getenv("DISABLE_CREWAI_IMPORTS") == "1":  # pragma: no cover - test guard
        raise ImportError("CrewAI imports disabled via env flag")
...
def validate_measurements(measurement_data: Dict | None = None, **kwargs) -> Dict:
    """Validate and normalize measurement data via the backend API."""

    payload = measurement_data or kwargs
    if not payload:
        raise ValueError("measurement_data payload is required")
    ...
    try:
        response = _post_with_retry(url, payload, validate_breaker)
...
def recommend_size(normalized_measurements: Dict | None = None, **kwargs) -> Dict:
    """Generate size recommendations from normalized measurements."""

    payload = normalized_measurements or kwargs
    if not payload:
        raise ValueError("normalized_measurements payload is required")
    ...
    try:
        response = _post_with_retry(url, payload, recommend_breaker)
    except requests.exceptions.Timeout:
...
recommend_sizes = recommend_size
