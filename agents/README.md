...
If you want to keep non-secret defaults (for example `AGENT_MODEL`), recreate `agents/.env` after the cleanup without the key. For this repo we currently keep `OPENAI_API_KEY` only in the GitHub Actions secret and export it manually when needed.
