...
const DEFAULT_LAB_URL = (process.env.NS_BRAND_URL as string) || 'http://localhost:3100';
const LAB_URL_KEY = 'brand.lab.url';
...
function wireWebView(page: Page) {
  ...
  webView.on('loadStarted', (event: LoadEventData) => {
    ...
    console.log(`[Brand Lab WebView] Loading ${event.url}`);
  });
  ...
  webView.on('loadFinished', (event: LoadEventData) => {
    ...
    if (event.error) {
      ...
      console.error(`[Brand Lab WebView] Failed loading ${event.url}: ${event.error}`);
    } else {
      model?.setStatus('');
      console.log(`[Brand Lab WebView] Loaded ${event.url}`);
    }
  });
}
...
function configureAndroidWebView(webView: WebView) {
  ...
  const ChromeClient = android.webkit.WebChromeClient.extend('FitTwinBrandLabChromeClient', {
    onPermissionRequest(request: android.webkit.PermissionRequest) {
      try {
        request.grant(request.getResources());
      } catch (err) {
        console.error('[Brand Lab WebView] Permission request failed', err);
      }
    },
